...
\end{codebox}
The \proc{Absorb-Left} and \proc{Absorb-Right} functions contract the triple $S*$, $O$, and $S$ to form respectively a new left and right environment, and \proc{Improve-Site} functions works (conceptually) by contracting $L$, $R$, and $O$ to form a matrix and then and using an eigensolver to find the minimum eigenvalue and corresponding eigenvector, starting with $S$ as a guess.
